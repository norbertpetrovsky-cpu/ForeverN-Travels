      ...
      <AnimatePresence initial={false}>
        {!collapsed && (
          <motion.div initial={{ height:0, opacity:0 }} animate={{ height:'auto', opacity:1 }} exit={{ height:0, opacity:0 }} transition={{ duration:0.22 }} style={{ overflow:'hidden' }}>
            ...
            {blockActivities.length === 0 && (
              <button onClick={() => onAdd(block.id)}
                style={{ width:'100%', padding:'6px 12px', borderRadius:8, background:'transparent', border:'1px dashed rgba(255,255,255,0.07)', color:'rgba(255,248,240,0.2)', fontSize:'0.68rem', cursor:'pointer', textAlign:'left', marginBottom:4 }}>
                + Pridať aktivitu
              </button>
            )}
          </motion.div>
    ...
    <div style={{ minHeight:'100vh', background:'radial-gradient(ellipse at 50% 0%,rgba(12,180,204,0.09) 0%,transparent 55%),linear-gradient(180deg,#0A1628 0%,#060D1A 100%)' }}>
      ...
      <style>{`
        @media(max-width:900px){
          .plan-dashboard { flex-direction: column !important; }
          .plan-right-col { position: static !important; width: 100% !important; max-height: none !important; overflow-y: visible !important; }
          .plan-main-col  { min-width: 0 !important; }
        }
      `}</style>

      {/* ── Top bar ── */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 24px 12px', maxWidth:1400, margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', gap:16 }}>
          ...
          <div>
            <div style={{ fontSize:'0.58rem', letterSpacing:'0.25em', textTransform:'uppercase', color:'#0CB4CC', opacity:0.75 }}>
              📅 Plán · {trip.destination}
            </div>
            <div style={{ fontSize:'1.1rem', fontWeight:900, color:'#fff', lineHeight:1.1 }}>Denný plán</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <span style={{ fontSize:'0.65rem', color:'rgba(255,248,240,0.28)', display:'none' }} className="plan-date-range">
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'0 24px 40px' }}>
        ...
        {activeDay && (
          <motion.div key={activeDay.id} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.3 }}>
            ...
            <div style={{
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <div>
                  <div style={{ fontSize:'0.58rem', textTransform:'uppercase', letterSpacing:'0.15em', color:'rgba(255,248,240,0.32)', fontWeight:700 }}>
                    Deň {activeDay.number} z {plan.days.length}
                  </div>
                  <h2 style={{ fontSize:'1.05rem', fontWeight:900, color:'#fff', margin:0, lineHeight:1.15 }}>
                    {fmtDateFull(activeDay.date)}
                  </h2>
                </div>
                {activeDay.label && (
                  <span style={{ fontSize:'0.7rem', fontWeight:700, color:'#3DCFE4', background:'rgba(12,180,204,0.12)', border:'1px solid rgba(12,180,204,0.28)', borderRadius:20, padding:'3px 12px' }}>
                    {activeDay.label}
                  </span>
                )}
                {totalCount > 0 && (
                  <span style={{ fontSize:'0.62rem', color:'rgba(255,248,240,0.32)', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:20, padding:'3px 10px' }}>
              ...
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                <button onClick={() => setShowTemplates(true)}
                  style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.65rem', fontWeight:600, color:'rgba(255,248,240,0.4)', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:20, padding:'4px 12px', cursor:'pointer' }}>
                  ✦ Šablóna
                </button>
                <button onClick={() => setShowEditDay(true)}
                  style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.65rem', fontWeight:600, color:'rgba(255,248,240,0.4)', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:20, padding:'4px 12px', cursor:'pointer' }}>
                  <Edit3 size={10}/> Upraviť deň
                </button>
                <button onClick={() => {
                    const src = activeDay
                  ...
                  style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.65rem', fontWeight:600, color:'rgba(255,248,240,0.4)', background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:20, padding:'4px 12px', cursor:'pointer' }}>
                  <Copy size={10}/> Duplikovať
                </button>
                <button onClick={() => setShowActivityModal({ block:'dopoludnie' })}
                  style={{ display:'flex', alignItems:'center', gap:5, fontSize:'0.65rem', fontWeight:700, color:'#3DCFE4', background:'rgba(12,180,204,0.12)', border:'1px solid rgba(12,180,204,0.3)', borderRadius:20, padding:'4px 14px', cursor:'pointer' }}>
                  <Plus size={11}/> Pridať aktivitu
                </button>
              </div>
            </div>

            {/* ── Two-column dashboard ── */}
            <div className="plan-dashboard" style={{ display:'flex', gap:16, alignItems:'flex-start' }}>

              {/* LEFT — main agenda (62%) */}
              <div className="plan-main-col" style={{ flex:'1 1 0', minWidth:0 }}>

                {totalCount === 0 && (
                  <div style={{ textAlign:'center', padding:'28px 20px', color:'rgba(255,248,240,0.25)', background:'rgba(255,255,255,0.02)', border:'1px dashed rgba(255,255,255,0.08)', borderRadius:14, marginBottom:12 }}>
                    <p style={{ fontSize:'1.8rem', marginBottom:6 }}>🌴</p>
                    <p style={{ fontSize:'0.82rem', fontStyle:'italic', marginBottom:14 }}>Voľný deň — oddych alebo pridaj prvý plán.</p>
                    <button onClick={() => setShowActivityModal({ block:'dopoludnie' })}
                      style={{ padding:'8px 22px', borderRadius:20, background:'rgba(12,180,204,0.14)', border:'1px solid rgba(12,180,204,0.35)', color:'#3DCFE4', fontSize:'0.78rem', fontWeight:700, cursor:'pointer' }}>
              ...
              {/* RIGHT — utility panels (38%), sticky */}
              <div className="plan-right-col" style={{
        ...
        {/* ── Ideas bucket — full-width horizontal strip at bottom ── */}
