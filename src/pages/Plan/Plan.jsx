...
// ─── Activity card ────────────────────────────────────────────
function ActivityCard({ activity, onEdit, onDelete, onToggleDone, onMoveBlock }) {
...
// ─── Time block section ───────────────────────────────────────
function TimeBlock({ block, activities, onAdd, onEdit, onDelete, onToggleDone, onMoveBlock }) {
...
// ─── Ideas bucket ─────────────────────────────────────────────
function IdeasBucket({ ideas, onChange, days, onAddToDay }) {
  const [adding, setAdding]   = useState(false)
  const [input, setInput]     = useState('')
  const [targetDay, setTarget]= useState(null)
  const inputRef = useRef(null)
  ...
  function addIdea() {
    const t = input.trim()
    if (!t) { setAdding(false); return }
    onChange([...ideas, { id:generateId(), title:t, category:'volny_cas', notes:'' }])
    setInput('')
  }
  ...
  return (
    <div style={{ background:'rgba(255,107,107,0.05)', border:'1px solid rgba(255,107,107,0.15)', borderRadius:18, padding:'18px 20px', marginTop:24 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
        <div>
          <div style={{ fontSize:'0.68rem', textTransform:'uppercase', letterSpacing:'0.18em', color:'rgba(255,107,107,0.7)', fontWeight:700 }}>
            💡 Nápady / Možno
          </div>
          <div style={{ fontSize:'0.62rem', color:'rgba(255,248,240,0.25)', marginTop:2 }}>Neplánované nápady, ktoré možno zaradíme</div>
        </div>
        <button onClick={() => setAdding(true)}
          style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px', borderRadius:20, fontSize:'0.68rem', fontWeight:700, color:'rgba(255,107,107,0.8)', background:'rgba(255,107,107,0.1)', border:'1px solid rgba(255,107,107,0.25)', cursor:'pointer' }}>
        ...
        {adding && (
          <motion.div initial={{ opacity:0, y:-4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
            ...
            <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              style={{ flex:1, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:9, padding:'8px 12px', color:'rgba(255,248,240,0.9)', fontSize:'0.8rem', fontFamily:'inherit', outline:'none' }}/>
            <button onClick={addIdea} style={{ width:32, height:32, borderRadius:8, background:'rgba(255,107,107,0.15)', border:'1px solid rgba(255,107,107,0.3)', color:'#FF6B6B', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Check size={13}/>
            </button>
            <button onClick={() => { setAdding(false); setInput('') }} style={{ width:32, height:32, borderRadius:8, background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,248,240,0.3)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <X size={13}/>
            </button>
          </motion.div>
        )}
      ...
      {ideas.length === 0 ? (
        <p style={{ fontSize:'0.72rem', color:'rgba(255,248,240,0.2)', fontStyle:'italic', textAlign:'center', padding:'8px 0' }}>
          Pridaj nápady, ktoré možno zaradíme neskôr.
        </p>
      ) : (
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {ideas.map(idea => (
            <div key={idea.id} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 12px', borderRadius:12, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)' }}>
              <span style={{ fontSize:'0.75rem', color:'rgba(255,248,240,0.7)', fontWeight:600 }}>{getCatIcon(idea.category)} {idea.title}</span>
              <button
                onClick={() => {
                  const dayNum = window.prompt(`Pridať "${idea.title}" do dňa (1–${days.length}):`)
                  const n = parseInt(dayNum)
                  if (!n || n<1 || n>days.length) return
                  const day = days[n-1]
                  onAddToDay(day.id, {
                ...
                title="Pridať do dňa"
                style={{ width:20, height:20, borderRadius:5, background:'rgba(12,180,204,0.15)', border:'1px solid rgba(12,180,204,0.3)', color:'#3DCFE4', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                <Plus size={10}/>
              </button>
              <button onClick={() => onChange(ideas.filter(x => x.id!==idea.id))}
                style={{ width:18, height:18, borderRadius:4, background:'none', border:'none', color:'rgba(255,248,240,0.2)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
                <X size={10}/>
              </button>
            </div>
          ))}
...
export default function Plan() {
  ...
  const [showActivityModal, setShowActivityModal] = useState(null)  // null | { act?, block? }
